...
# Global variables for live camera management
active_cameras = {}
...
def initialize_camera(camera_source):
    """Initialize camera and add it to active cameras"""
...
def generate_frames_live(camera_source, camera_location, camera_ip, camera_id=1):
    """Generate frames from live camera feed"""
    ...
    # Store camera in active cameras
    active_cameras[camera_id] = cap
    
    frame_count = 0
    fps = 30  # Default FPS for live camera
    frame_delay = 1.0 / fps
    ...
    try:
        while True:
            ...
            if not success:
                print(f"Failed to read from camera {camera_source}")
                break
...
# Camera streams - Updated to use live camera
@app.route('/main')
def kitchen_camera():
    # Use camera source 0 for default camera, or specify camera path/URL
    camera_source = 0  # Default camera (webcam)
    # camera_source = "/dev/video0"  # Linux camera device
    # camera_source = "rtsp://camera_ip:port/stream"  # IP camera RTSP stream
    
    camera_location = "main"
    camera_ip = "http://127.0.0.1:5000/main"
    camera_id = 1
    
    return Response(generate_frames_live(camera_source, camera_location, camera_ip, camera_id), 
